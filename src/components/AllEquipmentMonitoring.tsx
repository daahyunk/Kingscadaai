...
export function AllEquipmentMonitoring({ currentPressure, isAlarmActive }: AllEquipmentMonitoringProps) {
  ...
  const allEquipment: Equipment[] = [
    { id: 1, name: t('pump1'), type: 'pump', status: 'online', value: 13.2, unit: 'bar', threshold: 15.0, icon: Droplets },
    { id: 2, name: t('pump2'), type: 'pump', status: 'online', value: 14.1, unit: 'bar', threshold: 15.0, icon: Droplets },
    { id: 3, name: t('pump3'), type: 'pump', status: isAlarmActive ? 'alarm' : 'online', value: currentPressure, unit: 'bar', threshold: 15.0, icon: Droplets },
    { id: 4, name: t('pump4'), type: 'pump', status: 'online', value: 13.8, unit: 'bar', threshold: 15.0, icon: Droplets },
    { id: 5, name: t('temperatureSensorA'), type: 'temperature', status: 'online', value: 23.5, unit: '°C', threshold: 80.0, icon: Thermometer },
    { id: 6, name: t('temperatureSensorB'), type: 'temperature', status: 'online', value: 24.2, unit: '°C', threshold: 80.0, icon: Thermometer },
    { id: 7, name: t('flowMeter1'), type: 'flow', status: 'online', value: 150, unit: 'L/min', icon: Wind },
    { id: 8, name: t('flowMeter2'), type: 'flow', status: 'online', value: 145, unit: 'L/min', icon: Wind },
    { id: 9, name: t('pressureSensorA'), type: 'pressure', status: 'online', value: 12.8, unit: 'bar', threshold: 15.0, icon: Gauge },
    { id: 10, name: t('pressureSensorB'), type: 'pressure', status: 'online', value: 13.4, unit: 'bar', threshold: 15.0, icon: Gauge },
    { id: 11, name: t('pressureSensorC'), type: 'pressure', status: 'online', value: 14.0, unit: 'bar', threshold: 15.0, icon: Gauge },
    { id: 12, name: t('pressureSensorD'), type: 'pressure', status: 'online', value: 13.1, unit: 'bar', threshold: 15.0, icon: Gauge },
  ];
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur border border-slate-800 rounded-xl p-4">
      <h2 className="text-slate-300 mb-4">{t('allEquipment')}</h2>
              ...
              {isSelected && (
                <div className="mt-2 ml-4 p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    {equipment.threshold && (
                      <div>
                        <p className="text-slate-500 text-xs mb-1">{t('threshold')}</p>
                        <p className="text-slate-300">{equipment.threshold} {equipment.unit}</p>
                      </div>
                    )}
                    <div>
                      <p className="text-slate-500 text-xs mb-1">{t('status')}</p>
                      <p className={config.color}>{config.label}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs mb-1">{t('lastUpdated')}</p>
                      <p className="text-slate-300">{t('justNow')}</p>
                    </div>
                  </div>
                </div>
              )}
